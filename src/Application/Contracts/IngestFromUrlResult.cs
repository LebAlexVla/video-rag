...
public sealed record class IngestFromUrlResult
{
    public IngestFromUrlResult(
        bool success,
        ...
        ErrorInfo? error = null)
    {
        if (success)
        {
            if (string.IsNullOrWhiteSpace(sourceUrl))
                throw new ArgumentException("SourceUrl is required for successful URL ingest.", nameof(sourceUrl));

            if (string.IsNullOrWhiteSpace(localAudioPath))
                throw new ArgumentException("LocalAudioPath is required for successful URL ingest.", nameof(localAudioPath));

            if (string.IsNullOrWhiteSpace(lectureId))
                throw new ArgumentException("LectureId is required for successful URL ingest.", nameof(lectureId));

            if (string.IsNullOrWhiteSpace(lectureTitle))
                throw new ArgumentException("LectureTitle is required for successful URL ingest.", nameof(lectureTitle));

            if (string.IsNullOrWhiteSpace(transcriptPath))
                throw new ArgumentException("TranscriptPath is required for successful URL ingest.", nameof(transcriptPath));
